# QuickSell E-commerce Storefron

A high-performance, fully responsive e-commerce interface built with modern Vanilla JavaScript, HTML5, and CSS3. This project replicates a premium shopping experience with lightning-fast delivery and smart product management.

##  Features
- **Dynamic Product Rendering:** Automatically populates product grids and deals using centralized JavaScript data structures.
- **Category-Based Filtering:** Real-time product filtering across Electronics, Fashion, Home, and more via a seamless tab system.
- **Advanced Search & Sort:** millions of products searchable with category-specific logic and sorting by price or rating.
- **State-Managed Cart & Wishlist:** Fully functional sidebars that track item counts, quantities, and price totals in real-time.
- **Live Deal Countdown:** Integrated 8-hour countdown timer for flash sales with automatic UI updates.
- **Interactive UI Components:** Glassmorphism navbars, floating hero cards, hover-triggered product actions, and toast notifications.

##  Technical Stack
- **HTML5:** Semantic structure for accessible navigation and optimized SEO.
- **CSS3:** Custom properties (CSS variables), Grid Layout, Flexbox, and complex keyframe animations.
- **JavaScript (ES6+):** Dynamic DOM manipulation, state arrays for cart/wishlist, and event-based notification systems.
- **FontAwesome:** High-fidelity vector icons for UI elements and payment methods.

##  Project Structure
```text
├── index.html   # Main application structure, modals, and sidebar containers
├── style.css    # Custom design system, color palettes, and responsive grid
└── script.js    # Product data, state management, and UI logic functions

##  How to Use
Browse: Explore featured products or use the category grid to find specific item types.

Search: Use the search bar to find products by name or category across the entire inventory.

Shop: Add items to your cart or heart them to save to your wishlist for later.

Checkout: Manage quantities in the cart sidebar and proceed to the simulated checkout flow.

##  Future Roadmap
[1] Integration with a backend API (Node.js/Spring Boot) for real-time inventory.

[2] Persistence of user sessions and cart items using localStorage.

[3] Implementation of a real Payment Gateway (Stripe or Razorpay).

##  License
MIT

Created with HTML, CSS, and JavaScript
